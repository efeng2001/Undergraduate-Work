...
OS::OS(){
    ...
    wd.push_bottom(root_data);
    dataStack.push_bottom(root_data);
}
...
void OS::create_item(string fname, bool isFolder){
    if(search_item(fname) != NULL){
        ...
        Data* n_item = new Data;
        n_item -> name = fname;
        n_item -> size = 0;
        n_item -> isFolder = isFolder;
        if(isFolder){
            n_item -> childList = new MList;
        }else{
            n_item -> childList = NULL;
        }
        dataStack.push_top(n_item);
        wd.top()-> nodeData -> childList ->push_bottom(n_item);
...
void OS::file(string fname, bool isRead){
    if(search_item(fname) == NULL){
        ...
        search_item(fname) -> nodeData -> size = input.size();
    }
}
